...
export default function DonationForm() {
  ...
  function validate() {
    ...
    if (finalAmount < 1) nextErrors.amount = "Please enter a valid donation amount.";
      ...
      <div className="container-width grid gap-8 lg:grid-cols-[1fr_380px]">
        <form onSubmit={handleSubmit} className="rounded-[2rem] bg-white p-6 shadow-sm lg:p-10">
          ...
          <div className="mt-8">
            ...
            />
            {errors.amount && <p className="mt-2 text-sm text-coral">{errors.amount}</p>}
          </div>

          <div className="mt-6">
            <label className="label-style">Select cause</label>
            <select value={cause} onChange={(event) => setCause(event.target.value)} className="input-style">
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-2">
            <div>
              ...
              <input value={form.fullName} onChange={(e) => updateField("fullName", e.target.value)} className="input-style" />
              {errors.fullName && <p className="mt-2 text-sm text-coral">{errors.fullName}</p>}
            </div>
            <div>
              <label className="label-style">Email</label>
              <input value={form.email} onChange={(e) => updateField("email", e.target.value)} className="input-style" />
              {errors.email && <p className="mt-2 text-sm text-coral">{errors.email}</p>}
            </div>
            <div>
              <label className="label-style">Phone</label>
              <input value={form.phone} onChange={(e) => updateField("phone", e.target.value)} className="input-style" />
              {errors.phone && <p className="mt-2 text-sm text-coral">{errors.phone}</p>}
            </div>
            <div>
              <label className="label-style">Country</label>
              <input value={form.country} onChange={(e) => updateField("country", e.target.value)} className="input-style" />
            </div>
          </div>

          <div className="mt-6">
            <label className="label-style">Message</label>
            <textarea value={form.message} onChange={(e) => updateField("message", e.target.value)} rows={5} className="input-style" />
          </div>
        ...
        <DonationSummaryCard amount={finalAmount} donationType={donationType} cause={cause} />
      </div>
    </section>
  );
}
